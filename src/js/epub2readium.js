...
const {
    MediaOverlayNode,
} = require("r2-shared-js/dist/es8-es2017/src/models/media-overlay");
const {
    Publication,
} = require("r2-shared-js/dist/es8-es2017/src/models/publication");
const {
    Link,
} = require("r2-shared-js/dist/es8-es2017/src/models/publication-link");
const {
    AudioBookis,
...
const {
    isEPUBlication,
} = require("r2-shared-js/dist/es8-es2017/src/parser/epub");
const {
...
} = require("r2-lcp-js/dist/es8-es2017/src/parser/epub/lcp");
const {
    JsonArray,
    JsonMap,
    TaJsonDeserialize,
...
const {
    isHTTP,
} = require("r2-utils-js/dist/es8-es2017/src/_utils/http/UrlUtils");
const {
...
const {
    IStreamAndLength,
    IZip,
} = require("r2-utils-js/dist/es8-es2017/src/_utils/zip/zip");
const {
    Transformers,
} = require("r2-shared-js/dist/es8-es2017/src/transform/transformer");
...
const {
    zipHasEntry,
} = require("r2-shared-js/dist/es8-es2017/src/_utils/zipHasEntry");
...
function extractEPUB_ManifestJSON(pub, outDir, keys) {
    ...
    if (keys) {
        arrLinks.forEach((link) => {
            if (
                ...
                link.properties.encrypted.scheme ===
    ...
    arrLinks.forEach((link) => {
        if (
            link.properties &&
            link.properties.encrypted &&
            (link.properties.encrypted.algorithm ===
...
async function extractEPUB_Link(pub, zip, outDir, link) {
    ...
    if (!has) {
        console.log(
            `NOT IN ZIP (extractEPUB_Link): ${link.Href} --- ${hrefDecoded}`
        );
...
async function dumpPublication(publication) {
    ...
    const publicationJsonStr = global.JSON.stringify(
        publicationJsonObj,
        null,
        "  "
    );
    ...
    const publicationReverse = TaJsonDeserialize(
        publicationJsonObj,
        Publication
    );
    ...
    console.log(
        jsonDiff.diffString(publicationJsonObj, publicationJsonObjReverse)
    );
    ...
    if (publication.Spine) {
        for (const spineItem of publication.Spine) {
            ...
            if (spineItem.MediaOverlays) {
                ...
                const moReverse = TaJsonDeserialize(
                    moJsonObj,
                    MediaOverlayNode
                );
